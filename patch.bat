...
set md5check=
if not exist copy-to-device\%name%-patched.wad (set md5check=notexist) && (goto:miniskip)
...
if /i %md5check% EQU fail (support\sfk echo -spat \x20 \x20  [Yellow] MD5 check for existing file failed, repatching...) && (del copy-to-device\%name%-patched.wad)
if /i %md5check% EQU pass goto:skippatching
:miniskip
...
if /i %md5check% EQU pass (support\sfk echo -spat \x20 \x20  [Green] File is already patched and valid!) else (support\sfk echo -spat \x20 \x20  [Green] Patching done!)
echo.
set %variable%=
goto:setvariables
...
echo.
support\sfk echo -spat \x20 \x20  [Green] Everything done!
echo.
